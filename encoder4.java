...
public class encoder4{
  public static void main(String[] args) {
    ...
    try {
      ...
      int[] keyStream = new int[(8*plainText.size())/114*114+114];
      ...
      try {
        ...
        for (int i = 0; i < plainText.size(); i++) {
          keyStream[8*i] ^= (plainText.get(i) >> 7 & 1);
          keyStream[8*i+1] ^= (plainText.get(i) >> 6 & 1);
          keyStream[8*i+2] ^= (plainText.get(i) >> 5 & 1);
          keyStream[8*i+3] ^= (plainText.get(i) >> 4 & 1);
          keyStream[8*i+4] ^= (plainText.get(i) >> 3 & 1);
          keyStream[8*i+5] ^= (plainText.get(i) >> 2 & 1);
          keyStream[8*i+6] ^= (plainText.get(i) >> 1 & 1);
          keyStream[8*i+7] ^= (plainText.get(i) & 1);
        }
        int i = 0;
        while (i < keyStream.length) {
          if (i%114==112) {
            myWriter.write((byte) (keyStream[i] << 7 | keyStream[i+1] << 6));
            i+=2;
          }
          else {
            myWriter.write((byte) (keyStream[i] << 7 | keyStream[i+1] << 6 | keyStream[i+2] << 5 | keyStream[i+3] << 4 | keyStream[i+4] << 3 | keyStream[i+5] << 2 | keyStream[i+6] << 1 | keyStream[i+7]));
            i+=8;
          }
        }
        myWriter.close();
